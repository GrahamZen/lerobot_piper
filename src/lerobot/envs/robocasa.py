...
class RoboCasaEnv(RoboCasaGymEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ValueError(f"Unknown task '{task}'. Valid tasks are: {list(meta_info.keys())}") from err
    ...
    def get_abs_state(self) -> dict:
        """Snapshot robot joint state for failure recovery (arm + gripper only, NOT objects)."""
...
def _make_env_fns(
    *, task_name: str, n_envs: int, camera_names: list[str], gym_kwargs: Mapping[str, Any]
...
def create_robocasa_envs(
    ...
    for task in task_names:
        ...
        fns = _make_env_fns(
            task_name=task, n_envs=n_envs, camera_names=parsed_camera_names, gym_kwargs=gym_kwargs
        )
