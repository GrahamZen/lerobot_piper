    ...
    def __init__(
        ...
        *,
        ...
        if variant not in variant_to_constructor:
            raise ValueError(
                f"Unsupported V-JEPA 2 variant: {variant}. Supported: {sorted(variant_to_constructor.keys())}"
            )
        ...
        self.vjepa_model = constructor(
            patch_size=vit_patch_size, num_frames=tubelet_size, tubelet_size=tubelet_size
        )
