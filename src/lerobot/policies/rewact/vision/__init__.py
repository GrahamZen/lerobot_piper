    ...
    if vision_type == "resnet":
        ...
        vision_encoder: VisionEncoder = ResNetVisionEncoder(
            ...
            replace_final_stride_with_dilation=bool(
                getattr(config, "replace_final_stride_with_dilation", False)
            ),
        )
        ...
        if c.weights is None:
            raise ValueError(
                "vision_encoder_type='dinov3' requires config.dinov3.weights (local checkpoint path)."
            )
        ...
        if c.weights is None:
            raise ValueError(
                "vision_encoder_type='vjepa2' requires config.vjepa2.weights (local checkpoint path)."
            )
