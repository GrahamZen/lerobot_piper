...
class RewACTPolicy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer(
            "bin_edges", torch.linspace(config.value_min, config.value_max, config.num_value_bins + 1)
        )
    ...
    def select_action_and_reward_bins(
        self, batch: dict[str, Tensor], force_model_run: bool = False
    ) -> tuple[Tensor, dict]:
        """Select a single action given environment observations."""
        self.eval()
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        if self.config.image_features:
            ...
            if self.config.vision_encoder_type == "vjepa2":
                past_keys = [
                    k.replace("observation.images.", "observation.images_past.")
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        if "use_action_mask" in batch:
            action_mask = action_mask & batch["use_action_mask"].unsqueeze(-1).unsqueeze(-1)
        ...
        if "control_mode_autonomous" in batch:
            ...
            action_mask = action_mask & (
                ~batch["control_mode_autonomous"].squeeze().unsqueeze(-1).unsqueeze(-1)
            )
        ...
        if "reward" in batch and isinstance(batch["reward"], torch.Tensor):
            reward_targets = batch["reward"]
        elif "next.reward" in batch and isinstance(batch["next.reward"], torch.Tensor):
            reward_targets = batch["next.reward"]
        else:
            reward_targets = None

        if reward_targets is None or reward_targets.shape[0] != actions_hat.shape[0]:
            batch_size = actions_hat.shape[0]
            reward_targets = torch.zeros((batch_size, 1), device=actions_hat.device)
...
class RewACT(nn.Module):
    ...
    def __init__(self, config: RewACTConfig):
        ...
        if self.config.use_vae:
            ...
            self.vae_encoder_robot_state_input_proj = nn.Linear(self.state_dim, config.dim_model)
        ...
        # if self.config.robot_state_feature:
        self.encoder_robot_state_input_proj = nn.Linear(self.state_dim, config.dim_model)
        ...
        self.action_head = nn.Linear(config.dim_model, self.action_dim)
        ...
        self.register_buffer(
            "bin_values", torch.linspace(config.value_min, config.value_max, config.num_value_bins)
        )
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        if self.config.env_state_feature:
            encoder_input_tokens.append(self.encoder_env_state_input_proj(batch[OBS_ENV_STATE]))
