...
def _parse_ep_bounds(ep_meta: dict) -> tuple[int, int]:
    """Extract (from_idx, to_idx) from episode metadata, handling list or scalar."""
    from_idx = int(
...
def get_episode_bounds(dataset, episode_index: int) -> tuple[int, int]:
    ...
    return _parse_ep_bounds(dataset.meta.episodes[episode_index])
