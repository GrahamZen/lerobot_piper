...
This script allows you to step through the inference process one chunk at a time,
visualizing the inputs/outputs and executing the actions on the robot.

Key Features:
- Controlled Execution: Press RIGHT ARROW to trigger a single inference cycle.
- Full Cycle visualization: Observe -> Predict -> Execute.
- Rerun Integration: Visualizes valid observation images in Rerun.
- EMA Smoothing: Optional Exponential Moving Average smoothing on action chunks.

Arguments Explanation:
- --n_action_steps: Number of actions to execute from the predicted chunk (default: full chunk size).
                  Useful for executing fewer steps than predicted to increase replanning frequency.
- --ema: Enable Exponential Moving Average smoothing (values: true/false).
         Smoothes predictions across consecutive steps to reduce jitter.
- --ema_alpha: Smoothing factor for EMA (0 < alpha <= 1). Default 0.3.
               Smaller values = smoother (more lag), Larger values = closer to raw prediction.
- --cameras: JSON string to override camera configuration.
             Example: '{"left":{"type":"opencv","index_or_path":"/dev/video14","width":640,"height":480,"fps":60,"rotation":0},...}'

Usage:
  uv run python tools/step_through_model.py \
      --pretrained_path /path/to/model/checkpoints/last/pretrained_model \
      --repo_id local/lerobot_pick_cups \
      --task "Pick up the cups" \
      --cameras='{"left":{"type":"opencv","index_or_path":"/dev/video14","width":640,"height":480,"fps":60,"rotation":0}}'

Controls:
  RIGHT ARROW : Step (Observe -> Predict -> Execute)
  ESC         : Quit
"""
...
DEFAULT_PRETRAINED_MODEL_PATH = Path(
    "/home/droplab/workspace/lerobot_piper/outputs/train/lerobot_long_horizon_cup_torque_variation/checkpoints/last/pretrained_model"
)
DEFAULT_DATASET_ROOT = "~/.cache/huggingface/lerobot/local/"  # Uses HF_LEROBOT_HOME default
DEFAULT_DATASET_ID = "local/lerobot_long_horizon_cup_torque_variation1"
