"""
This script provides an interactive Tkinter-based GUI to step through frames of a LeRobot episode.
It allows users to visualize camera streams alongside recorded observations and actions.

Key Features:
- Multi-camera playback: Displays all synchronized camera views for each frame.
- State & Action display: Shows formatted vector data (e.g., joint positions, velocities).
- Interactive navigation: Keyboard shortcuts for stepping through or jumping to specific frames.
- Automatic Path Construction: Joins a hardcoded base path with the provided repository ID.

Usage:
    python tools/viz/step_through_episode.py \
        --repo_id local/lerobot_cup_and_saucer \
        --episode 0

Controls:
    Right Arrow / Space  → Next frame
    Left Arrow           → Previous frame
    Home                 → Jump to first frame (index 0)
    End                  → Jump to last frame
    Escape               → Quit the browser
"""
