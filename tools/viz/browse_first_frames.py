"""
This script iterates through all episodes in a LeRobot dataset and logs the first frame
of each episode to Rerun. This is useful for quickly verifying camera alignment,
starting conditions, or dataset diversity across many episodes.

Key Features:
- Dataset-wide sweep: Automatically finds and processes every episode in the repository.
- Rerun Timeline: Logs each first frame to a dedicated "episode" timeline for easy scrubbing.
- Multi-camera support: Captures and displays all available camera views for the first frame.
- Format agnostic: Handles both raw bytes and tensor-based image data.

Usage:
    uv run python tools/viz/browse_first_frames.py --repo_id local/lerobot_pick_cups_aug
"""
